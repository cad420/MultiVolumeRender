...
ScalarAndVectorFieldRenderImpl::ScalarAndVectorFieldRenderImpl(int w, int h)
...
void ScalarAndVectorFieldRenderImpl::SetScalarFieldData(ScalarFieldData data) {
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RED, data.x,data.y,data.z, 0, GL_RED,
...
void ScalarAndVectorFieldRenderImpl::SetVectorFieldDAta(VectorFieldData data) {
    ...
    glBufferData(GL_ARRAY_BUFFER,data.vertices.size()*sizeof(std::array<float,3>),data.vertices.data(),GL_STATIC_DRAW);
